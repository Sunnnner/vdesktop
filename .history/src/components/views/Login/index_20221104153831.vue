<template>
    <n-card >
        <n-row>
            <h3>编辑配置信息：</h3>
            
            <n-col :span="24">
                <n-form ref="formRef" :model="model" :rules="rules">
                    <n-form-item path="appid" label="APPID">
                        <n-input v-model:value="model.appid"  placeholder="输入appid" />
                    </n-form-item>
                    <n-form-item path="secret" label="SECRET">
                        <n-input v-model:value="model.app_secret"  placeholder="输入app secret"  />
                    </n-form-item>
                    <n-form-item path="host" label="域名">
                        <n-input v-model:value="model.host"  placeholder="输入host"  />
                    </n-form-item>
                    <n-form-item path="vm" label="默认VM">
                        <n-input v-model:value="model.default_vm"  placeholder="输入默认启动的虚拟主机"  />
                    </n-form-item>
                    <n-row :gutter="[0, 24]">
                        <n-col :span="18">
                            <div style="display: flex; justify-content: flex-end">
                                <n-button @click="openFile"
                                    round type="primary">
                                    导入配置文件
                                </n-button>
                            </div>
                        </n-col>
                        <n-col :span="6">
                            <div style="display: flex; justify-content: flex-end">
                                <n-button
                                    :disabled="model.app_secret === null && model.default_vm === null && model.appid === null"
                                    round type="primary">
                                    保存
                                </n-button>
                            </div>
                        </n-col>
                    </n-row>
                </n-form>
            </n-col>
        </n-row>
    </n-card>

</template>

<script lang="ts" setup>
import { ref } from 'vue'
import {
    FormInst,
    FormRules
} from 'naive-ui'
import { invoke } from '@tauri-apps/api/tauri';
import { open } from '@tauri-apps/api/dialog';


interface ModelType {
    appid: string | null
    app_secret: string | null
    host: string | null
    default_vm: string | null
}
interface DataValue{
    appid: string | null
    appsecret: string | null
    url: string | null
    'default-vm': string | null
}

const formRef = ref<FormInst | null>(null)
const modelRef = ref<ModelType>({
    appid: null,
    app_secret: null,
    host: "https://vdesk.knd.io",
    default_vm: null
})
const model = modelRef.value


const rules: FormRules = {
    appid: {
        required: true,
        message: '请输入appid',
        trigger: 'blur'
    },
    secret: {
        required: true,
        message: '请输入app secret',
        trigger: 'blur'
    },
    host: {
        required: true,
        message: '请输入host',
        trigger: 'blur'
    },
    vm: {
        required: true,
        message: '请输入默认启动的虚拟主机',
        trigger: 'blur'
    }
}

const openFile = async () => {
    const selected = await open({
    filters: [{
        name: 'vdesk',
        extensions: ['yaml', 'yml']
    }]
    });
    if (selected) {
    // user selected multiple files
        invoke('read_yaml_file', {path: selected}).then(
            (data: DataValue) => {
                modelRef.value.appid = data?data.appid: ''
                modelRef.value.app_secret = data.appsecret
                modelRef.value.host = data.url
                modelRef.value.default_vm = data['default-vm']
            }
        )
        const data = await invoke('read_yaml_file', {path: selected})
    }  else {
    // user selected a single file
    }
}

</script>